...
public class TestUtils extends TestSetUp {
	...
	@DataProvider(name = "MenuBarData")
	public Object[][] getData1(Method method) {
	...
	public Object[][] getData(Method m) {
		...
		int testRows = 0;
		while (!excel.getCellData(configProperty.getProperty("testSheetName"), 0, dataStartRowNum + testRows)
				.equals("")) {
		...
		for (int rNum = dataStartRowNum; rNum < (dataStartRowNum + testRows); rNum++) {
			...
			for (int cNum = 0; cNum < testCols; cNum++) {
				...
				// cNum,rNum)); String colName =
				excel.getCellData(configProperty.getProperty("testSheetName"), cNum, colStartColNum);
